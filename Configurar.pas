...
type
  ...
  public
    procedure CarregarConfiguracoes(AJson: TJsonValue);
...
uses
  JsonUtils, ShellApi, Vcl.Dialogs;
...
procedure TfrmConfiguracao.CarregarConfiguracoes(AJson: TJsonValue);
var
...
begin
  ...
  try
    edtUrl.text := TJsonUtil.LerJson(ADicionario, 'RedmineUrl');
    edtKey.text := TJsonUtil.LerJson(ADicionario, 'RedmineKey');
    edtIdProjeto.text := TJsonUtil.LerJson(ADicionario, 'IdProjeto');
    edtVersao.text := TJsonUtil.LerJson(ADicionario, 'IdVersao');
    edtNomeVersao.text := TJsonUtil.LerJson(ADicionario, 'NomeVersao');
    edtAgente.text := TJsonUtil.LerJson(ADicionario, 'Agent');
    edtStatus.text := TJsonUtil.LerJson(ADicionario, 'Status');
    edtCategoria.text := TJsonUtil.LerJson(ADicionario, 'Categoria');
    edtPrioridade.text := TJsonUtil.LerJson(ADicionario, 'Prioridade');
    edtDataInicial.Date := ISO8601ToDate(TJsonUtil.LerJson(ADicionario, 'DataInicial'));
    edtDataFinal.Date := ISO8601ToDate(TJsonUtil.LerJson(ADicionario, 'DataFinal'));
  finally
